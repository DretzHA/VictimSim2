...
def victims_clustering(merged_victims):
    ...
    cluster_array = np.empty((0,2), float) #array para features do cluster - numero colunas = total de features utilizadas
    ...
    for seq, data in merged_victims.items():
        ...
        cluster_array = np.vstack([cluster_array, coord_array]) #passa valores para cluster array (nesse caso, somente as posições)
